...
main(int argc, char *argv[])
{
  ...
  fprintf(stderr, "now is in main in part3-b-4\n");
  {
    char dd[512];
    sprintf(dd, "%s/da%d", argv[2], getpid());
    fprintf(stderr, "now is in main in part3-b-5\n");
    if(access(dd, 0) != 0){
  ...
  fprintf(stderr, "now is in main in part3-b-6\n");
  setbuf(stdout, 0);
